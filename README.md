...
*   **Persistenza AOF (Append-Only File):** Implementata una strategia di persistenza base AOF. Ogni comando che modifica i dati (`SET`, `DEL`, `HSET`, etc.) viene salvato in un file `appendonly.aof`. All'avvio, il server ricarica questo file per ripristinare lo stato dei dati.
...
*   **Persistenza Semplice:** È stata implementata una strategia di persistenza AOF di base. Non sono presenti meccanismi di riscrittura o compattazione del file AOF, che quindi crescerà indefinitamente con il tempo.